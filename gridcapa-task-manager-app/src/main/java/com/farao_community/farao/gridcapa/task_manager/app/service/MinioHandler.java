...
public class MinioHandler {
                    ...
                    if (!processFile.isInputFile()) {
                        ...
                        if (!runningOrPendingTasks.isEmpty()) {
                            ...
                            Set<TaskWithStatusUpdate> taskWithStatusUpdates = taskService.addProcessFileToTasks(processFile,
                                                                                                                processFileMinio.getFileEventType(), true, true);
    ...
    private ProcessFileMinio buildProcessFileMinioFromEvent(Event event) {
        ...
        if (!isEmpty(validityInterval))  {
            ...
            String fileType = metadata.get(FILE_TYPE_METADATA_KEY);
            String documentId = metadata.get(DOCUMENT_ID_METADATA_KEY);
    ...
    ProcessFileMinio getProcessFileMinio(final OffsetDateTime startTime, final OffsetDateTime endTime,
                                         final String objectKey, final String fileType,
                                         final String fileGroup, final String documentId) {
        /*
    ...
    private void removeWaitingFileWithSameTypeAndValidity(ProcessFileMinio newProcessFileMinio) {
        Set<ProcessFileMinio> processFileMiniosWaiting = waitingFilesList.stream()
    ...
    private boolean isFileValidForTimestamp(final OffsetDateTime timestamp, final ProcessFile processFile) {
